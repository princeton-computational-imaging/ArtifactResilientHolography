"""
This is the script that is used for the implementation of HoloNet [Peng et al. 2020]. The class HoloNet(nn.module) is 
...
class HoloNet(nn.Module):
    ...
    def __init__(self, distance=0.1, wavelength=520e-9, feature_size=6.4e-6,
                 ...
                 manual_aberr_corr=False, phase_shift = 0, inference=False):
    ...
    def forward(self, target_amp, flag_debug = False):
        ...
        if (self.zernike is None and self.source_amp is None
            ...
            slm_amp_phase = torch.cat((amp, ang, self.zernike,
                                        self.source_amp), -3)
        if self.inference:
            return amp, self.final_phase_only(slm_amp_phase), slm_naive
        else:
