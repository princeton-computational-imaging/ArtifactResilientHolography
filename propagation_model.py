"""
This is the script that is used for the parameterized wave propagation.

This implementation is from Neural-Holography [Peng et al. 2020].
"""
...
class ModelPropagate(nn.Module):
    ...
    def forward(self, phases, skip_lut=False, skip_tm=False):
        ...
        processed_complex = torch.complex(real, imag)
        precomped_H = self.precomped_H
        output_complex = self.prop(processed_complex,
                                    self.feature_size,
                                    self.wavelength,
                                    self.distance,
                                    precomped_H=precomped_H,
